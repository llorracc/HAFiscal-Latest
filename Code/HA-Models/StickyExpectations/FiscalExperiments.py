    ...
    if run_models:
        ...
        if run_parker or run_tax_cut:
            ...
            if run_parker:
                ...
                cLvl_StickyNone_parker, cLvl_StickyBonus, cLvl_FrictionlessNone_parker, cLvl_FrictionlessBonus = runParkerExperiment(StickySOmarkovEconomy,Bonus,1,8,True) # One quarter ahead
                t_end = time()
                ...
                t_start = time()               
                # Run Parker experiments for different lead times for the policy
                cLvl_StickyNone_parker_splurge, cLvl_StickyBonus_splurge, cLvl_FrictionlessNone_parker_splurge, cLvl_FrictionlessBonus_splurge = runParkerExperiment(StickySOmarkovEconomy,(1-splurge)*Bonus,1,8,True) # One quarter ahead
                t_end = time()
                print('Running the "Parker experiment" with splurge took ' + str(t_end-t_start) + ' seconds.')
                # add splurge to consumption
                pLvlBase = StickySOmarkovEconomy.agents[0].PlvlAggNow # Average permanent income level
                BonusLvl = Bonus*StickySOmarkovEconomy.wRte*pLvlBase # Bonus check of 5% of average quarterly income   
                cLvl_StickyBonus_splurge[1] = cLvl_StickyBonus_splurge[1] + splurge*BonusLvl
                cLvl_FrictionlessBonus_splurge[1] = cLvl_FrictionlessBonus_splurge[1] + splurge*BonusLvl               
                ...
                plt.plot(cLvl_StickyBonus_splurge/cLvl_StickyNone_parker_splurge, label="Sticky, with splurge")
                plt.plot(cLvl_FrictionlessBonus_splurge/cLvl_FrictionlessNone_parker_splurge, label="Frictionless, with splurge")
                plt.title('Consumption in case of bonus / in case of no bonus')
                plt.legend(loc="upper right")
                plt.show()
                
                plt.plot((cLvl_StickyBonus-cLvl_StickyNone_parker)/BonusLvl, label="Sticky")
                plt.plot((cLvl_FrictionlessBonus-cLvl_FrictionlessNone_parker)/BonusLvl, label="Frictionless")
                plt.plot((cLvl_StickyBonus_splurge-cLvl_StickyNone_parker_splurge)/BonusLvl, label="Sticky, with splurge")
                plt.plot((cLvl_FrictionlessBonus_splurge-cLvl_FrictionlessNone_parker_splurge)/BonusLvl, label="Frictionless, with splurge")
                plt.legend(loc="upper right")
                plt.title('Additional consumption relative to Bonus')
                plt.show()
