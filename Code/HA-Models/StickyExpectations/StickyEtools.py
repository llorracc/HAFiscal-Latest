...
def runStickyEregressionLagged(infile_name,interval_size,meas_err,sticky,all_specs):
    '''
    Runs regressions to test how persistent consumption growth is. 
    The idea is that, in contrast to Reis' model, our model dynamics last longer 
    than 4 quarters
    ...
    lags = [0,4,8,12]
    lag_coeffs_array = np.zeros((4,7))
    for i in range(len(lags)):
        this_lag = lags[i]
        # Determine how many subsample intervals to run (and initialize array of coefficients)
        N = DeltaLogC.size//interval_size # (// rounds down)
        CoeffsArray = np.zeros((N,7)) # Order: DeltaLogC_OLS, DeltaLogC_IV, DeltaLogY_IV, A_OLS, DeltaLogC_HR, DeltaLogY_HR, A_HR
        StdErrArray = np.zeros((N,7)) # Same order as above
        RsqArray = np.zeros((N,5))
        PvalArray = np.zeros((N,5))
        ...
        for n in range(N):
            ...
            # Run OLS on log consumption
            mod = sm.OLS(DeltaLogC_n[1+this_lag:],sm.add_constant(DeltaLogC_n[0:-1-this_lag]))
            res = mod.fit()
            CoeffsArray[n,0] = res._results.params[1]
            StdErrArray[n,0] = res._results.HC0_se[1]
            RsqArray[n,0] = res._results.rsquared_adj
            ...
            # Define instruments for IV regressions
            temp = np.transpose(np.vstack([DeltaLogC_n[1:-3-this_lag],DeltaLogC_n[:-4-this_lag],DeltaLogY_n[1:-3-this_lag],DeltaLogY_n[:-4-this_lag],A_n[1:-3-this_lag],A_n[:-4-this_lag],Delta8LogC_n[1:-3-this_lag],Delta8LogY_n[1:-3-this_lag]]))
            instruments = sm.add_constant(temp) # With measurement error
    
            # Run IV on log consumption
            mod = sm.OLS(DeltaLogC_n[3:-1-this_lag],instruments)
            res = mod.fit()
            DeltaLogC_predict = res.predict()
            mod_2ndStage = sm.OLS(DeltaLogC_n[4+this_lag:],sm.add_constant(DeltaLogC_predict))
            res_2ndStage = mod_2ndStage.fit()
            mod_IV = smsrg.IV2SLS(DeltaLogC_n[4+this_lag:], sm.add_constant(DeltaLogC_n[3:-1-this_lag]),instruments)
            res_IV = mod_IV.fit()
            CoeffsArray[n,1] = res_IV._results.params[1]
            StdErrArray[n,1] = res_IV.bse[1]
            RsqArray[n,1] = res_2ndStage._results.rsquared_adj
            ...
            # Run IV on log income
            if this_lag>0:
                mod = sm.OLS(DeltaLogY_n[4:-this_lag],instruments)
            else:
                mod = sm.OLS(DeltaLogY_n[4:],instruments)
            res = mod.fit()
            DeltaLogY_predict = res.predict()
            mod_2ndStage = sm.OLS(DeltaLogC_n[4+this_lag:],sm.add_constant(DeltaLogY_predict))
            res_2ndStage = mod_2ndStage.fit()
            if this_lag>0:
                mod_IV = smsrg.IV2SLS(DeltaLogC_n[4+this_lag:], sm.add_constant(DeltaLogY_n[4:-this_lag]),instruments)
            else:
                mod_IV = smsrg.IV2SLS(DeltaLogC_n[4+this_lag:], sm.add_constant(DeltaLogY_n[4:]),instruments)
            res_IV = mod_IV.fit()
            CoeffsArray[n,2] = res_IV._results.params[1]
            StdErrArray[n,2] = res_IV.bse[1]
            RsqArray[n,2] = res_2ndStage._results.rsquared_adj
            ...
            # Run IV on assets
            mod = sm.OLS(A_n[3:-1-this_lag],instruments)
            res = mod.fit()
            A_predict = res.predict()
            mod_2ndStage = sm.OLS(DeltaLogC_n[4+this_lag:],sm.add_constant(A_predict))
            res_2ndStage = mod_2ndStage.fit()
            mod_IV = smsrg.IV2SLS(DeltaLogC_n[4+this_lag:], sm.add_constant(A_n[3:-1-this_lag]),instruments)
            res_IV = mod_IV.fit()
            CoeffsArray[n,3] = res_IV._results.params[1]
            StdErrArray[n,3] = res_IV.bse[1]
            RsqArray[n,3] = res_2ndStage._results.rsquared_adj
            ...
            # Run horserace IV
            regressors = sm.add_constant(np.transpose(np.array([DeltaLogC_predict,DeltaLogY_predict,A_predict])))
            mod_2ndStage = sm.OLS(DeltaLogC_n[4+this_lag:],regressors)
            res_2ndStage = mod_2ndStage.fit()
            if this_lag>0:
                mod_IV = smsrg.IV2SLS(DeltaLogC_n[4+this_lag:], sm.add_constant(np.transpose(np.array([DeltaLogC_n[3:-1-this_lag],DeltaLogY_n[4:-this_lag],A_n[3:-1-this_lag]]))),instruments)
            else:
                mod_IV = smsrg.IV2SLS(DeltaLogC_n[4+this_lag:], sm.add_constant(np.transpose(np.array([DeltaLogC_n[3:-1-this_lag],DeltaLogY_n[4:],A_n[3:-1-this_lag]]))),instruments)
            res_IV = mod_IV.fit()
            CoeffsArray[n,4] = res_IV._results.params[1]
            CoeffsArray[n,5] = res_IV._results.params[2]
            CoeffsArray[n,6] = res_IV._results.params[3]
        ...
        # Make results 
        lag_coeffs_array[i,:] = np.mean(CoeffsArray,axis=0)
    return lag_coeffs_array
