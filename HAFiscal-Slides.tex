...
\begin{itemize} 
	...
	\begin{itemize}
		\itemsep = .25\bigskipamount 
		\item Consumers subject to transitory/permanent income shocks and recessions (during which aggregate demand multipliers exist)
		\item Consumers heterogenous in education, income, wealth, emplyoment status 
		\item Consistent with evidence on \textit{intertemporal marginal propensity to consume} (iMPC)  (based on rich micro data from Norway)
	\end{itemize}
\end{itemize}
\end{frame}
...
\begin{itemize}
	\itemsep = .25\bigskipamount 
	\item Spending multipliers (for unfinanced policies)
	\item Welfare (cost of policy considered)
\end{itemize}
...
\begin{frame}
\frametitle{Impulse responses for the three policies}
