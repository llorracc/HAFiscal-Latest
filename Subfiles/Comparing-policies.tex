...
\begin{figure}[htb]
	...
	\begin{subfigure}[b]{.33\linewidth}
		...
		\caption{payroll tax cut}
		\notinsubfile{\label{fig:recessiontaxcutrelrecession}}
	\end{subfigure}
	\caption{Impulse responses of aggregate income and consumption to policy shocks during recessions with and without aggregate demand effects{\label{fig:Policyrelrecession}}}
	%\parbox{16cm}{\small \vspace{.15cm} \textbf{Note}: \normalsize}
\end{figure}
...
full-horizon) cost of the policies.


\begin{figure}[t]
  \centering
  \includegraphics[width=0.8\linewidth]{Code/HA-Models/FromPandemicCode/Figures/Cummulative_multipliers}
  \caption{Cumulative multiplier as a function of the horizon for the three policies.}
  \parbox{16cm}{\small \vspace{.15cm} \textbf{Note}: Policies are implemented during a recession with aggregate demand effect active.\normalsize}
...
\begin{table}[ht] 
	\center
	\input{\econtexRoot/Code/HA-Models/FromPandemicCode/Tables/CRRA2/welfare6.tex}
...
\begin{table}[ht] 
	...
	\caption{Welfare measure, calculated for policies implemented both out of and in a recession with and without aggregate demand effects. The values outside of the brackets capture the multipliers in the model without the splurge, while those inside the brackets are mulitipliers with the splurge.}
