...
The splurge is essential in matching the empirical evidence mentioned above. If we impose a zero splurge in our estimation, the model can match the iMPC only by reducing liquid assets to levels inconsistent with data. This is because generating a high MPC in the absence of the splurge is only possible by reducing the average distance to the borrowing constraint in the population. Moreover, as established in \citet{fagereng_mpc_2021}, also the wealthy exhibit a substantial marginal propensity to consume out of transitory income shocks. Without the splurge, the wealthy in the model instead smooth consumption very strongly over time. Hence, the splurge is required to account for high initial MPCs across the wealth distribution.
