...
First, we take from \citet{fagereng_mpc_2021} the marginal propensity to consume out of a one-period income shock. We target not only the initial (aggregate) response of consumption to the income shock, but also the subsequent effect on consumption in years one through four after the shock as well as heterogeneous, initial consumption response across quartiles of liquid wealth. The shares of lottery winnings expended at different time horizons, as found in \citet{fagereng_mpc_2021}, are plotted in figure \ref{fig:aggmpclotterywin}. Table \ref{tab:ComparisonSplurge} (last row) shows the initial consumption response across liquid wealth quartiles. 
...
\begin{figure}[th]
  \begin{center}
    \includegraphics[width=.9\textwidth]{../Figures/LorenzPoints_CRRA_2.0_R_1.01.pdf}
