...
\begin{table}[t]
  \center
  \input{\econtexRoot/Code/HA-Models/Target_AggMPCX_LiquWealth/Figures/MPC_WealthQuartiles_Table.tex}
...
Finally, we consider implications of our model for two different patterns of spending over time. The first is the dynamics of spending after a lottery win from \citet{fagereng_mpc_2021}. This pattern was used in the estimation of the splurge factor in section~\ref{sec:splurge}, but was not targeted when estimating the discount factor distributions for each education group in section~\ref{sec:estimBetas}. Figure~\ref{fig:USaggmpclotterywin} shows that the model that is estimated taking the value of the splurge as given, results in a distribution of spending over time that is very similar to the one found in the Norwegian data. 

Figure~\ref{fig:expiryUI} shows the pattern of income and spending of a household that becomes unemployed and remains unemployed long enough unemployment benefits to expire. \citet{ganongConsumer2019} report that nondurable spending drops by 12 percent the month when benefits expire. Our quarterly model is broadly consistent with this pattern, and the drop in spending the quarter after the expiry of UI benefits is 18 percent. 
