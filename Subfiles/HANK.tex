...
\textbf{Comparison with \cite{broer2025stimulus}}: As discussed earlier, \cite{broer2025stimulus} also compute fiscal multipliers for commonly implemented stimulus policies in a HANK and SAM framework. A key distinction is that our model exhibits more spending out of income shocks in the quarters immediately following the quarter of the shock. As shown in \cite{auclertIntertemporal}, accounting for the path of iMPCs beyond the first quarter significantly amplifies cumulative fiscal multipliers. By capturing this persistence—consistent with microeconomic evidence—our model produces larger multipliers than those in \cite{broer2025stimulus}.
