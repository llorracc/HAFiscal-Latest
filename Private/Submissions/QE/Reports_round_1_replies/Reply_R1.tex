...
\item \textit{\textbf{Additional comments}} 
	...
	\begin{enumerate}
		...
		Finally, in a robustness section that we have included in this version, we consider some of the general equilibrium effects of stimulus policies in a HANK-SAM model. In that framework we also considered a policy of increasing UI generosity and found that \hl{the multipliers for a policy of increasing the replacement rates for unemployed households are substantially smaller than an extension of benefit eligibility}. We have not included these results in the paper, but Figure~XX shows the multipliers of the two policies.
		% CDC: add citation to Ravn and Sterk 2017
