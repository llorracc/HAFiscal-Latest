...
\item \textit{\textbf{Additional comments}} 
	...
	\begin{enumerate}
		...
		\paragraph{Response.} Our calibration strategy for the separation and job-finding rates is to match the data that we have from the Bureau of Labor Statistics. This includes average unemployment rates by education groups, and the average duration of unemployment spells, but it does not include data on education-specific average durations. As we are using the 2004 wave of the SCF, we also use the average number of weeks unemployed from 2004 which was 19.6 weeks, almost exactly 1.5 quarters. This pins down the job-finding rate for all education groups, and the education-specific separation rates are then calibrated to match the education-specific unemployment rates. 
		
		This calibration is also consistent with the results in \citet{mincer1991education} who finds that the main difference between education groups is in the incidence of unemployment, and not its duration. He states that ``the reduction	of the incidence of unemployment is found to be far more		important than the reduced duration of unemployment in creating	the educational differentials in unemployment rates'' (page 1). \citeauthor{mincer1991education} reports probabilities of transitioning into unemployment as the product of the probability of job separation times the conditional probability of unemployment given a job separation. Both of these are lower for higher education levels. For our calibration, this means that a higher job finding rate \textit{within} the quarter of the job separation for more educated workers translates into a lower probability of transitioning from employment to unemployment. 
		
		Admittedly, \citet{mincer1991education} refers to data from 1979 and 1980, and there may have been substantial changes in the labor market since then, but nevertheless, our calibration strategy is consistent both with his results and the data we have. 
